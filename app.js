...
async function checkBackend(showSuccess = true) {
  ...
  try {
    ...
    const health = await KtdBridge.call('healthCheck');
    const bootstrap = await KtdBridge.call('publicBootstrap');
    venues = Array.isArray(bootstrap?.venues) && bootstrap.venues.length ? bootstrap.venues : STATIC_VENUES.slice();
    ...
    if (showSuccess) console.error(err);
    return false;
  }
}
...
function metaGet(key) { return reqP(store(STORE_META).get(key)).then(v => v ? v.value : null); }
function metaSet(key, value) { return reqP(store(STORE_META, 'readwrite').put({key, value})); }
function metaDel(key) { return reqP(store(STORE_META, 'readwrite').delete(key)); }
function queuePut(row) { return reqP(store(STORE_QUEUE, 'readwrite').put(row)); }
function queueDelete(id) { return reqP(store(STORE_QUEUE, 'readwrite').delete(id)); }
function queueAll() { return reqP(store(STORE_QUEUE).getAll()).then(x => x || []); }
function photoPut(row) { return reqP(store(STORE_PHOTOS, 'readwrite').put(row)); }
function photoGet(id) { return reqP(store(STORE_PHOTOS).get(id)); }
function photoAll() { return reqP(store(STORE_PHOTOS).getAll()).then(x => x || []); }
